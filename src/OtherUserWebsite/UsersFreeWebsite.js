...
export default function UsersWebsite() {
  ...
  const [FreeWebsite, setAllWebsites] = useState([]);
  ...
  //   const filteredData = FreeWebsite.filter((website) => {
  ...
  return (
    <Page title="Purchase Free Website" style={{ paddingLeft: '10px', paddingRight: '10px' }}>
      {loading ? (
        ...
        <>
          ...
          {FreeWebsite.length > 0 ? (
            <>
              {FreeWebsite.map((website) => (
